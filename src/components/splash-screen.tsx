  ...
  return (
    <AnimatePresence>
      {!splashDone && (
        ...
        >
          {/* Ambient gold orbs that breathe — neon-sign glow */}
          <motion.div
            className="absolute top-1/4 left-1/3 w-[520px] h-[520px] rounded-full blur-3xl pointer-events-none"
            style={{ background: "radial-gradient(circle, rgba(255,215,0,0.18), transparent 65%)" }}
            animate={{ scale: [1, 1.15, 1], opacity: [0.5, 0.8, 0.5] }}
            transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
          />
          <motion.div
            className="absolute bottom-1/4 right-1/3 w-[460px] h-[460px] rounded-full blur-3xl pointer-events-none"
            style={{ background: "radial-gradient(circle, rgba(255,215,0,0.12), transparent 65%)" }}
          ...
          <div className="relative flex flex-col items-center">
            {/* Steam wisps rising above the emblem */}
            <div className="relative h-16 w-32 mb-[-10px]">
            ...
            {/* The SpagKing emblem — neon glow + flicker */}
            <motion.div
            ...
            >
              {/* Glow behind emblem */}
              <div className="absolute inset-0 blur-2xl opacity-70 animate-glow-pulse">
            ...
            {/* Wordmark in script font */}
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1, delay: 1.4, ease: [0.22, 1, 0.36, 1] }}
              className="mt-8 wordmark text-4xl sm:text-5xl"
            >
              SpagKing
            </motion.div>
            ...
            <motion.p
              ...
              className="mt-3 text-[11px] sm:text-xs text-muted-foreground font-light uppercase"
          ...
          {/* Brand sparkle dots */}
          <motion.div className="absolute top-12 left-12 w-1 h-1 rounded-full bg-[var(--gold)]" animate={{ opacity: [0, 1, 0], scale: [0.5, 1.2, 0.5] }} transition={{ duration: 2, repeat: Infinity, delay: 1 }} />
          <motion.div className="absolute top-20 right-16 w-1 h-1 rounded-full bg-[var(--gold)]" animate={{ opacity: [0, 1, 0], scale: [0.5, 1.2, 0.5] }} transition={{ duration: 2.4, repeat: Infinity, delay: 1.6 }} />
          <motion.div className="absolute bottom-24 left-20 w-1 h-1 rounded-full bg-[var(--gold)]" animate={{ opacity: [0, 1, 0], scale: [0.5, 1.2, 0.5] }} transition={{ duration: 2.2, repeat: Infinity, delay: 2.2 }} />
          <motion.div className="absolute bottom-32 right-24 w-1 h-1 rounded-full bg-[var(--gold)]" animate={{ opacity: [0, 1, 0], scale: [0.5, 1.2, 0.5] }} transition={{ duration: 2.6, repeat: Infinity, delay: 1.4 }} />
        </motion.div>
      )}
    </AnimatePresence>
  );
